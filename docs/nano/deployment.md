...
Status 2026-09-18 07:08 UTC: **deployed incl. the nowcast forecast and the motion field on the
observed frames.** Production runs `ghcr.io/fuekiin/brightsky:sha-4ceae3b` (`nano-health` tip,
tag `nano-v6`) with the third container `brightsky-radar3d-1`; records in `bright_sky_config` `docs/deploy-2026-09-18-radar3d.md`
(rain/clouds/cells, `nano-v3`), `docs/deploy-2026-09-18-forecast.md` (model forecast, `nano-v4`)
`docs/deploy-2026-09-18-nowcast.md` (nowcast blend, `nano-v5`) and
`docs/deploy-2026-09-18-motion.md` (motion field, `nano-v6`).
...
| CPU | rain 3–13 s per 5-min cycle; cloud frame ~0.5 s per cycle; ICON run load ~3–4 min per 3 h (374 GRIB decodes per step × 6 steps, in the ICON thread); forecast (nowcast) frames rewritten every cycle: ~20–30 s per 5-minute cycle on the dev Mac (12 warps of the 24-slab observed volume + the model blends); cells negligible |
| Disk | rain 15.3 MB + clouds 7.6 MB + flow 1.3 MB per 5 min → ~0.9 GB at 3 h retention; forecast ~150 MB per key (12 × 12.7 MB), a new key every cycle, the newest two kept (~300 MB); raw sweeps/GRIBs transient (≤ ~1 GB during an ICON download; downloads stop below `RADAR3D_MIN_FREE_GB` = 5 GB free) |
