...
async def deliver(conn, client, *, device_id, environment, token,
                  ...
                  expiration=None, now, sleep=asyncio.sleep):
    ...
    for delay in (*RETRY_DELAYS, None):
        ...
        if not result.retryable or delay is None:
            break
    ...
    if result.ok:
        ...
        if result.token_dead:
            await forget_token(conn, device_id, token_field)
...
async def forget_token(conn, device_id, token_field):
    if token_field == 'apns_token':
        logger.info('Deleting device %s: APNs token is dead', device_id)
        await conn.execute(
            'DELETE FROM push.devices WHERE id = $1', device_id)
